...
class UltimateSubPro(ctk.CTk):
    def __init__(self):
        ...
        self.status = ctk.CTkLabel(self, text="Моля, въведете данните си в 'Настройки'", font=("Arial", 12))
    ...
    def setup_settings_ui(self):
        ...
        self.api_entry = ctk.CTkEntry(frame, width=450, placeholder_text="Въведи API Key...")
        ...
        self.user_entry = ctk.CTkEntry(up, placeholder_text="Потребител")
        self.user_entry.insert(0, self.config.get("username", ""))
        self.user_entry.pack(side="left", padx=5)
        
        self.pass_entry = ctk.CTkEntry(up, show="*", placeholder_text="Парола")
    ...
    def login(self):
        if not self.api_entry.get() or not self.user_entry.get():
            messagebox.showwarning("Внимание", "Моля, попълнете API Key и Потребител в 'Настройки'!")
            return

        def t():
            h = {'Api-Key': self.api_entry.get().strip(), 'Content-Type': 'application/json', 'User-Agent': 'SubApp v1.0'}
            try:
                r = requests.post("https://api.opensubtitles.com/api/v1/login", 
                                  json={"username": self.user_entry.get().strip(), "password": self.pass_entry.get().strip()}, 
                                  headers=h)
                if r.status_code == 200: 
                    self.token = r.json().get('token')
                    self.status.configure(text="✅ Успешен вход в OpenSubtitles!", text_color="green")
                else: 
                    self.status.configure(text=f"❌ Грешка при вход: {r.status_code}", text_color="red")
            except: 
                self.status.configure(text="❌ Няма връзка с интернет.", text_color="red")
        threading.Thread(target=t, daemon=True).start()

    def search(self):
        movie = self.search_entry.get().strip()
        if not movie: return
        for w in self.scroll_bg.winfo_children(): w.destroy()
        for w in self.scroll_en.winfo_children(): w.destroy()
        def t():
            h = {'Api-Key': self.api_entry.get().strip(), 'User-Agent': 'SubApp v1.0'}
            try:
    ...
    def dl(self, fid, tdir, tname):
        h = {'Api-Key': self.api_entry.get().strip(), 'Authorization': f'Bearer {self.token}', 'Content-Type': 'application/json', 'User-Agent': 'SubApp v1.0'}
        try:
    ...
    def start_deepl_thread(self): 
        if not self.deepl_entry.get():
            messagebox.showwarning("Грешка", "Въведете DeepL API Key в настройките!")
            return
        threading.Thread(target=self.run_deepl, daemon=True).start()

    def run_deepl(self):
        try:
            self.d_start.configure(state="disabled")
            t = deepl.Translator(self.deepl_entry.get().strip())
